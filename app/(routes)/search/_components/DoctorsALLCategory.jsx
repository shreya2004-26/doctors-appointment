...
const DoctorsALLCategory = () => {
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      <div className="hidden md:flex flex-row items-center border-b  w-full">
      ...
      <h2 className="hidden md:block text-xs text-slate-500">Suggestions</h2>
      <div className="flex mt-5 md:mt-0 flex-row md:flex-col w-screen md:w-fit gap-5 md:gap-5 overflow-x-auto">
        {categories === null
          ...
          : categories?.map((curr, index) => (
              <Link
                href={`/search/${curr?.slug}`}
                key={index}
                className={`flex gap-2 items-center  py-2 pr-10 pl-2 rounded-md cursor-pointer ${
                  curr?.slug === path
