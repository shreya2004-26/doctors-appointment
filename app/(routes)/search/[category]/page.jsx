...
const page = () => {
  ...
  const getSortedDoctorsList = async () => {
    const query =
      gql`
        query MyQuery {
  doctors(where: {categories_some: {slug: "` +
      slug +
      `"}}) {
    name
    id
    ...
    <div className="grid grid-cols-1 md:grid-cols-5 gap-10">
      ...
      <div className="col-span-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
          {doctors === null ? (
            Array.from({ length: 4 }).map((curr, index) => {
              return (
                <Skeleton
                  key={index}
                  className="h-[390px] w-full rounded-xl mt-5"
                />
              );
            })
          ) : doctors?.length == 0 ? (
            <h1 className="text-[16px] text-gray-600 mt-5">
              No doctors available!{" "}
            </h1>
          ) : (
            doctors?.map((curr, index) => {
              return <DoctorsSortedList key={index} data={curr} />;
            })
          )}
        </div>
      </div>
