    ...
    <div className="flex gap-5 border py-5 px-8 w-[320px] md:w-[1300px] justify-between rounded-xl ">
      <div className="flex flex-col md:flex-row gap-3 md:gap-8">
        <Image
          ...
          width={2000}
          height={200}
          className="w-[70px] h-[70px] object-cover rounded-full self-center"
          alt="no image"
        />
        <div className="flex flex-col gap-3">
          <h2 className="font-bold text-lg"> {doctors?.name}</h2>
          <h2 className=" flex gap-1 text-gray-500 text-base">
            {" "}
            <MapPin className="w-5 h-5 text-primary" /> {doctors?.address}
          </h2>
          <h2 className="flex gap-1 text-sm">
            <Calendar className="w-4 h-4 text-primary mx-1" /> Appointment On:{" "}
            {moment(appointment_date).format("DD MMM YYYY")}
          </h2>
          <h2 className="flex gap-1 text-sm">
            <Clock className="w-4 h-4 text-primary mx-1" /> At Time :{" "}
            {appointment_time}
          </h2>
        </div>
      </div>
      <Button
        ...
        variant="ghost"
        className="hidden lg:block border border-primary text-primary hover:bg-primary hover:text-white self-center"
      >
        Cancel Appointment
      </Button>
    </div>
